...
public class DownloadUtil {
    ...
    public static void downloadFile(InputStream inputStream, String fileName, HttpServletResponse resp, HttpServletRequest req) {
        ...
        try {
            ...
            resp.setHeader("content-disposition", "attachment;filename=" + fileName);
            // 设置响应内容的类型
            resp.setContentType("application/x-excel" + "; charset=" + charsetCode);
            // 设置响应内容的长度

            // 输出
            outStream(inputStream, resp.getOutputStream());
        } catch (Exception e) {
            System.out.println("执行downloadFile发生了异常：" + e.getMessage());
        }
    }
